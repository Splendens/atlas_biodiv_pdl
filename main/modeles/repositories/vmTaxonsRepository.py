...
# get nbs taxon proteges communes
def getNbTaxonsProCommunes(connection, insee):
    sql = """
        SELECT  count(distinct o.cd_ref) AS nb_taxon_pro   
        FROM atlas.vm_observations o
            JOIN atlas.vm_taxons t ON t.cd_ref=o.cd_ref
        WHERE protection_stricte = 'oui'        
            AND o.insee = :thisInsee
    """
    req = connection.execute(text(sql), thisInsee=insee)
    taxonProCom = dict()
    for r in req:
...
# get nbs taxon patrimoniaux communes
def getNbTaxonsPatriCommunes(connection, insee):
    sql = """
        SELECT  count(distinct o.cd_ref) AS nb_taxon_patri   
        FROM atlas.vm_observations o
            JOIN atlas.vm_taxons t ON t.cd_ref=o.cd_ref
        WHERE patrimonial = 'oui'        
            AND o.insee = :thisInsee
    """
    req = connection.execute(text(sql), thisInsee=insee)
    taxonPatriCom = dict()
...
# get nbs taxon proteges epci
def getNbTaxonsProEpci(connection, nom_epci_simple):
    sql = """
        SELECT  count(distinct o.cd_ref) AS nb_taxon_pro  
        FROM atlas.vm_observations o
            JOIN atlas.vm_taxons t ON t.cd_ref=o.cd_ref
            ...
            JOIN atlas.vm_epci e ON ec.id = e.id
        WHERE protection_stricte = 'oui'        
            AND e.nom_epci_simple = :thisNomEpciSimple
    """
    req = connection.execute(text(sql), thisNomEpciSimple=nom_epci_simple)
    taxonProEpci = dict()
    for r in req:
...
# get nbs taxon patrimoniaux epci
def getNbTaxonsPatriEpci(connection, nom_epci_simple):
    sql = """
        SELECT  count(distinct o.cd_ref) AS nb_taxon_patri  
        FROM atlas.vm_observations o
            JOIN atlas.vm_taxons t ON t.cd_ref=o.cd_ref
    ...
    return taxonPatriEpci





# get nbs taxon proteges departement
def getNbTaxonsProDpt(connection, num_dpt):
    sql = """
        SELECT  count(distinct o.cd_ref) AS nb_taxon_pro  
        FROM atlas.vm_observations o
                    JOIN atlas.vm_taxons t ON t.cd_ref=o.cd_ref
                    WHERE left(o.insee,2)::int = :thisNumdpt
        AND protection_stricte = 'oui'      
    """
    req = connection.execute(text(sql), thisNumdpt=num_dpt)
    taxonProDpt = dict()
    for r in req:
...
# get nbs taxon patrimoniaux departement
def getNbTaxonsPatriDpt(connection, num_dpt):
    sql = """
        SELECT  count(distinct o.cd_ref) AS nb_taxon_patri 
        FROM atlas.vm_observations o
                    JOIN atlas.vm_taxons t ON t.cd_ref=o.cd_ref
