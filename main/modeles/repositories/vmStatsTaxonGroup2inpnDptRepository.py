...
def getStatsTaxonGroup2inpnDptChilds(connection, num_dpt):
      sql = """
         SELECT   
            acanthocephales, 
            algues_brunes, 
            ...
            FROM atlas.vm_stats_espece_group2inpn_dpt group2inpn
            WHERE group2inpn.num_dpt::int = :thisNumdpt
        """.encode('UTF-8')

      mesGroup = connection.execute(text(sql), thisNumdpt=num_dpt)
      grouplist = []
