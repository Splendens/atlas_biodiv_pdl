...
var communeLayer = L.geoJson(epciGeoJson, {
	style : function(){
		return {
			fillColor: 'blue',
			opacity: 1,
...
if (configuration.PRESSION_PROSPECTION){
	...
	$.ajax({
	  url: configuration.URL_APPLICATION+'/api/pressionProspectionEpci/'+nom_epci_simple, 
	  dataType: "json",
	  beforeSend: function(){
	    loader.show(); 
	  }
    ...
    var SwitcherCommControl = L.Control.extend({
      ...
      onAdd: function (map) {
        ...
        switcherCommContainer.onclick = function(){
              ...
              $.ajax({
					  url: configuration.URL_APPLICATION+'/api/pressionProspectionEpciMaillesCommunales/'+nom_epci_simple, 
					  dataType: "json",
					  beforeSend: function(){
					    loader.show(); 
					  }
